...
function getPHQ9Interpretation(score) {
    ...
    if (score >= 15) return {
        ...
        color: "text-red-500",
        action: "Active treatment recommended."
    };

    if (score >= 10) return {
        severity: "Moderate depression",
        color: "text-yellow-600",
        action: "Counseling or follow-up recommended."
    };
...
function renderCalendar(elementId, date, checkedDays = []) {
    ...
    const monthYearDisplayId = elementId
        ...
        .replace('streak-calendar-days', 'streak-current-month-year');
    ...
    for (let i = 1; i <= lastDay.getDate(); i++) {
        ...
        if (elementId === 'calendar-days') {
            ...
            dayEl.addEventListener('click', () => {
                ...
                document
                    .querySelectorAll(`#${elementId} .calendar-day`)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const messageBox = document.getElementById('message-box');
    const closeMessageBox = document.getElementById('close-message-box');   // FIXED

...
/**
 * Utility function for AI Chat formatting
 */
function formatTextToHtml(text) {
    let paragraphs = text.split('\n\n');
    let html = paragraphs.map(p => {
...
function renderCalendar(elementId, date, checkedDays = []) {
    ...
    const displayId = elementId.includes('streak') ? 'streak-current-month-year' : 'current-month-year';
    ...
    for(let i = 1; i <= lastDay.getDate(); i++) {
        ...
        dayEl.className = 'calendar-day bg-gray-100';
        dayEl.textContent = i;
        if (checkedDays.includes(i)) dayEl.classList.add('checked-in-day');
...
function showPage(pageId) {
    ...
    // Render calendars only when needed
    if (pageId === 'book-session-page') renderCalendar('calendar-days', currentDate);
    if (pageId === 'daily-checkin-page') renderCalendar('streak-calendar-days', streakDate, checkedInDays);
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Home Page Quick Buttons
    document.getElementById('start-chat-btn')?.addEventListener('click', () => showPage('ai-chat-page'));
    document.getElementById('book-session-btn')?.addEventListener('click', () => showPage('book-session-page'));
    document.getElementById('explore-resources-btn')?.addEventListener('click', () => showPage('resource-hub-page'));
    document.getElementById('checkin-now-btn')?.addEventListener('click', () => showPage('daily-checkin-page'));
    document.getElementById('account-btn')?.addEventListener('click', () => showPage('login-page'));

    // PHQ-9 Scoring
    document.getElementById('phq9-form')?.addEventListener('submit', (e) => {
        e.preventDefault();
        const formData = new FormData(e.target);
        let score = 0;
        for (let i = 1; i <= 9; i++) {
            score += parseInt(formData.get(`phq9_q${i}`) || 0);
        }
        const interp = getPHQ9Interpretation(score);
        const resDiv = document.getElementById('phq9-result');
        resDiv.innerHTML = `<h3 class="${interp.color}">Score: ${score} - ${interp.severity}</h3><p>${interp.action}</p>`;
...
function createPost() {
    const text = document.getElementById('postInput').value;
    const mood = document.getElementById('moodSelect')?.value || "😊 Happy";
    const isAnon = document.getElementById('anonCheck')?.checked;

    if (!text.trim()) return alert("Please type something first!");

    const feed = document.getElementById('postFeed');
    const postHTML = `<div class="pa-card bg-white p-4 rounded-xl shadow mb-4">
        <strong>${isAnon ? 'Anonymous' : 'Student'}</strong> <span class="text-purple-500">${mood}</span>
        <p class="mt-2">${text}</p>
    </div>`;
